...
{
    class avia_sc_caorda_testimonial_slider extends aviaShortcodeTemplate
    {
        /**
        ...
        {
            $this->config['name']			= __('Testimonial Slider', 'avia_framework' );
            $this->config['tab']			= __('Testimonial Elements', 'avia_framework' );
            $this->config['icon']			= AviaBuilder::$path['imagesURL']."sc-contentslider.png";
            $this->config['order']			= 83;
            $this->config['target']			= 'avia-target-insert';
            $this->config['shortcode'] 		= 'av_testimonial_slider';
...
if ( !class_exists( 'avia_content_slider' ) )
{
    class avia_content_slider
    {
        ...
        public function html()
        {
            ...
            $thumb_fallback = "";
            $output .= "<div {$data} class='avia-testimonial-content-slider avia-content-slider-element-container avia-content-slider-element-{$type} avia-content-slider avia-smallarrow-slider avia-content-{$type}-active avia-content-slider".avia_content_slider::$slider." avia-content-slider-{$total} {$class}' {$styling}>";
            ...
            while($testimonials->have_posts()): $testimonials->the_post();
                ...
                $output .= "<section class='testimonial-entry slide-entry flex_column {$post_class} {$grid} {$extraClass}' $markup>";
                ...
                $feat_image = wp_get_attachment_image_src( get_post_thumbnail_id());
                $output .= !empty($feat_image[0]) ? "<div class='testimonial-image' $markup>" : '';
                $output .= (!empty($feat_image[0]) && !empty($title)) ? '<img class="testimonial-image-bordered" src="'.$feat_image[0].'" />' :'';
                $output .= !empty($feat_image[0]) ? '</div>' : '';

                $output .= !empty($title) ? "<h3 class='testimonial-title slide-entry-title entry-title' $markup>" : '';
                ...
                $output .= !empty($content) ? "<div class='slide-entry-excerpt entry-content' $markup>".ShortcodeHelper::avia_apply_autop(ShortcodeHelper::avia_remove_autop($content))."</div>" : "";

                $testimonial_owner = get_post_meta(get_the_ID(),'Testimonial_client_name',true);
                $output .= !empty($testimonial_owner) ? "<div class='testimonial-owner' $markup>- ".$testimonial_owner."</div>" : "-adasd";
