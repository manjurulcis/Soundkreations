...
if ( !class_exists( 'avia_sc_caorda_instructors_slider' ) )
{
	class avia_sc_caorda_instructors_slider extends aviaShortcodeTemplate
	{
			/**
			...
			{
				$this->config['name']			= __('Instructors Slider', 'avia_framework' );
				$this->config['tab']			= __('Content Elements', 'avia_framework' );
				$this->config['icon']			= AviaBuilder::$path['imagesURL']."sc-image.png";
				$this->config['order']			= 100;
				$this->config['target']			= 'avia-target-insert';
				$this->config['shortcode'] 		= 'caorda:instructors_slider';
				$this->config['modal_data']     = array('modal_class' => 'mediumscreen');
				$this->config['tooltip'] 	    = __('Inserts a slider element containing all instructors', 'avia_framework' );
			}
			...
			function editor_element($params)
			{
				...
				$params['innerHtml'] .= "<h3>Instructors Slider</h3>";
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				$output = '<div class="instructors-container">';
				...
				while ($instructors->have_posts()) : $instructors->the_post();
                    ...
                    $output .= '</div>';
					$output .= '<div class="instructor-content">';
					if (has_excerpt()) :
                        $output .= get_the_excerpt();    
                    else :
                        $output .= implode(' ', array_slice(explode(' ', get_the_content()), 0, 25)) . '...';
                    endif;
					$output .= '</div>';
				...
				endwhile;
				$output .= '</ul>';
                $output .= '<div class="instructors-navigation">';
                $output .= '<a href="#prev" class="instructors-prev"></a><a href="#next" class="instructors-next"></a>';
                $output .= '</div>';
				$output .= '</div>';
				wp_reset_query();
				return $output;
