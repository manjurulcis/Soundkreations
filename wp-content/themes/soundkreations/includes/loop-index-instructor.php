...
if (have_posts()) :
	...
	echo "<article class='".implode(" ", get_post_class('post-entry post-entry-type-'.$post_format . " " . $post_class . " ".$with_slider))."' ".avia_markup_helper(array('context' => 'entry','echo'=>false)).">";
        ...
        if(strpos($blog_style, 'big') !== false)
        {
            ...
            if (get_post_meta('quote',true) != '') :
            echo '<div class="quote">'.get_post_meta('quote',true).'</div>';
            endif;
            if (get_post_meta('signature',true) != '') :
            echo '<div class="signature"><img src="'.get_post_meta('signature',true).'" alt="'.get_the_title().'" /></div>';
            endif;
            echo '</div>';
        }
